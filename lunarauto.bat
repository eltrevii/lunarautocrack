...
fc "%~f0" "dum2.bat">nul || (goto doupdate)
exit /b
...
if exist %_lunar.path.raw%\username.txt (
	for /f %%i in ('type %_lunar.path.raw%\username.txt') do (if not "%%i"=="" (set "_lastuname=%%i"))
...
if exist %_lunar.path.raw%\ram.txt (
	for /f %%i in ('type %_lunar.path.raw%\ram.txt') do (if not "%%i"=="" (set "_lastram=%%i"))
